...
import { getHeureMiroirHubCards } from '@/lib/heuresMiroirsFlammeJumelle';
...
const config: ContentPageConfig = {
  ...
  header: {
    ...
    anchors: [
      ...
      { href: '#heures-miroirs', label: 'Heures miroirs' },
  ...
  faq: [
    {
      q: 'Flamme jumelle ou relation toxique, comment savoir ?',
    ...
    {
      q: 'Combien de temps dure la séparation flamme jumelle ?',
    ...
    <ContentPage config={config}>
      ...
      <article className="bg-white rounded-xl shadow-md p-8 mb-8 border-t-4 border-purple-500">
        <p className="text-lg leading-relaxed mb-4">
          La <strong>flamme jumelle</strong> est le concept spirituel le plus intense de l'<Link href="/glossaire/amour-karmique/" className="text-purple-600 hover:text-purple-800 underline font-medium">amour karmique</Link> : une seule âme originelle divisée en deux corps distincts, destinée à se retrouver pour accomplir une mission d'évolution supérieure. Contrairement à l'<strong>âme s&oelig;ur</strong>, qui offre harmonie et confort, la <strong>connexion de flamme jumelle</strong> agit comme un miroir impitoyable de vos <Link href="/glossaire/ombre-shadow-work/" className="text-purple-600 hover:text-purple-800 underline font-medium">ombres</Link>, de vos blessures et de vos potentiels inexploités. Cette relation déclenche un <strong>voyage spirituel</strong> profond, jalonné de phases de fusion intense, de séparation douloureuse et de <strong>retrouvailles karmiques</strong> transformatives.
        </p>
        <p className="text-lg leading-relaxed mb-4">
      ...
      <section id="signes" className="bg-white rounded-xl shadow-md p-8 mb-8">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-cyan-50 border-l-4 border-cyan-500 p-5 rounded-lg">
            ...
            <p className="text-gray-700">
              Les <Link href="/glossaire/synchronicite-amoureuse/" className="text-purple-600 hover:text-purple-800 underline font-medium">synchronicités</Link> se multiplient de manière spectaculaire : chiffres miroirs (11:11, 22:22), coïncidences impossibles, signes répétés dans votre environnement. L'univers semble orchestrer votre rencontre et confirmer votre lien par des <strong>messages cosmiques</strong> de plus en plus évidents. Ces signes s'amplifient autour des moments clés de votre relation. Chaque <a href="#heures-miroirs" className="text-purple-700 underline hover:text-purple-900">heure miroir</a> porte d'ailleurs un message différent selon la phase que vous traversez.
            </p>
          </div>
      ...
      {/* Heures miroirs : parent du cluster /flamme-jumelle/[heure]/ */}
      <section id="heures-miroirs" className="mb-8 rounded-xl border border-gray-200 bg-white p-6 md:p-8">
        <h2 className="mb-4 text-2xl font-bold text-gray-900">Signification des heures miroirs pour les flammes jumelles</h2>
        <p className="mb-4 text-lg font-semibold text-gray-800">
          Une heure miroir ne dit pas ce que fait votre jumeau : elle nomme la phase que vous traversez. 12h12 parle d&apos;alignement, 17h17 d&apos;épreuve, 20h20 de message, 10h10 de nouveau cycle, 23h23 de lâcher-prise et 21h21 d&apos;introspection.
        </p>
        <p className="mb-6 text-gray-700">
          Les <strong>heures miroirs</strong> sont les synchronicités les plus fréquemment rapportées par les personnes qui vivent une <strong>connexion de flamme jumelle</strong>. Chacune possède sa lecture propre dans la tradition, son nombre et son arcane du <strong>tarot</strong>, et chacune se lit différemment selon que vous êtes en séparation, en silence radio ou en retrouvailles. Voici les six heures les plus consultées, chacune détaillée sur sa propre page.
        </p>

        <div className="grid gap-3 sm:grid-cols-2">
          {getHeureMiroirHubCards().map((c) => (
            <Link
              key={c.slug}
              href={c.href}
              className="block rounded-lg border border-gray-200 p-4 transition hover:border-purple-400"
            >
              <div className="font-bold text-purple-700">{c.heure} signification flamme jumelle</div>
              <div className="text-sm text-gray-600">{c.theme}</div>
            </Link>
          ))}
        </div>
